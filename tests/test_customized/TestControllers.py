...
class TestControllers(BaseTestCase):
    def create_customize_test_form(self, commit_hash, platform, commit_select=['', '']):
    ...
    def test_customize_test_fails_with_wrong_commit_hash(self, mock_user, mock_git, mock_requests):
        ...
        with self.app.test_client() as c:
            response = c.post(
                '/account/login', data=self.create_login_form_data(self.user.email, self.user.password))
            response = c.post(
                '/custom/', data=self.create_customize_test_form('', ['linux']), follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            self.assert_template_used('custom/index.html')
            self.assertIn('Commit hash is not filled in', str(response.data))
            response = c.post(
                '/custom/', data=self.create_customize_test_form('test-url', ['linux']))
    ...
    def test_customize_test_creates_with_right_test_commit(self, mock_user, mock_git, mock_requests):
        ...
        with self.app.test_client() as c:
            response = c.post(
                '/account/login', data=self.create_login_form_data(self.user.email, self.user.password))
            response = c.post(
                '/custom/', data=self.create_customize_test_form('abcdef', ['linux']), follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            self.assert_template_used('custom/index.html')
